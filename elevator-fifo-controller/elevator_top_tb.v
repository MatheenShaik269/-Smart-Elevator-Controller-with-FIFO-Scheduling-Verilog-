...
always @(posedge clk)
begin
    $display("T=%0t | rst=%b elec=%b gen=%b |floor=%0d CUR_FLOOR=%0d TARGET=%0d req=%b current_state=%s | UP=%b DOWN=%b | DO=%b DC=%b | L=%b F=%b ALARM=%b | estop=%b weight=%0d",
        $time,rst,electricity,generator,
        floor,
        dut.current_floor,   // current floor
        dut.target_floor,                  //  target floor
        request_valid,
        state,
...
weight=500; send_request(4'd3); #200;
weight=400; #300;
...
//TC21
$display("\n=== TC21: Multiple calls (inside + outside the lift) ===");
send_request(4'd12);
down=1'b1;send_request(4'd9);down=1'b0;
#800;

//TC22
$display("\n=== TC22: Multiple calls from outside the lift ===");
up=1'b1;send_request(4'd12);up=1'b0;
down=1'b1;send_request(4'd9);down=1'b0;
#500;
